...
A zero-dependency, vanilla HTML/CSS/JS dashboard that puts a visual face on every feature of the Rust backend. No build step, no framework, no install — just open a browser.
...
> Browsers block `fetch()` calls from `file://` pages. Any local HTTP server works — `python3 -m http.server`, `npx serve`, VS Code Live Server, etc.
...
```
frontend/
├── index.html   — Page structure, all 8 sections, 2 modals
├── style.css    — Dark-theme design system, all component styles
├── app.js       — All API calls, render logic, event wiring
└── package.json — Optional npm scripts (npx serve / python3)
```
...
- **Last benchmark summary** — winners for insert, lookup, and iterate with timestamps
...
**Table columns:** truncated ID · name · category badge · price · quantity · created date · actions

**Actions per row:**
- **View** — opens a modal with all product fields plus a live **per-set lookup timing panel** showing how long `HashSet`, `IndexSet`, and `BTreeSet` each took to find this product (in µs), and whether it was found in each set
- **Edit** — pre-fills the create form with current values; submits a `PUT`
- **Delete** — confirmation prompt, then `DELETE`; table reloads instantly

**New Product button** — modal form with name, category select, price (cents), quantity, and optional description.
...
- **Quick buttons:** 100 · 1 k · 5 k · 10 k · 25 k · 50 k (click to jump the slider)
- **Seed button** — `POST /api/seed?count=N`
- **Result card** on success:
  - Seeded count
  - Total products now in the database
  - Seed time (ms)
  - Set sync time (ms) — how long it took to load all products into all three in-memory sets

---

### ⚡ Benchmark

The core feature — runs `SetManager::run_benchmark()` against all products in the database and displays the results visually.

**Buttons:**
- **▶ Run Benchmark** — `POST /api/benchmark/run` (may take a moment on large datasets)
- **↺ Reload Last** — `GET /api/benchmark/report` — restores the last run without re-running
...
|---|---|
| Colored top border | Blue = HashSet · Green = IndexSet · Purple = BTreeSet |
| 🏆 Winner badges | Shown on the card that won each category |
| Description | Plain-English summary of the set's characteristics |
| Bar charts | Proportional bars for insert / lookup hit / lookup miss / iterate all / remove half — shorter = faster |
| Exact values | ms for bulk ops, µs for single lookups |
| Order indicator | ✅ guaranteed order or 🔀 arbitrary, with order type label |
| Iteration sample | Expandable list of the first 10 names iterated — proves order behavior |
...
A live snapshot of the three in-memory sets — the clearest way to see iteration order differences.

Calls `GET /api/benchmark/sets/status` and renders three columns:

| Column | Color | Order |
|---|---|---|
| HashSet | Blue | 🔀 Arbitrary (hash-based) |
| IndexSet | Green | ✅ Insertion order (FIFO) |
| BTreeSet | Purple | ✅ Alphabetical by name |
...
**Configuration form:**
- **Concurrency** (1–200): number of virtual users running in parallel
- **Ops per user** (1–1000): operations each user performs
- **Pre-seed count** (optional): seeds N products before the run starts

**Operation mix (backend-defined):** 50% reads · 25% creates · 15% updates · 10% deletes
Deletes only target products created *during the same run* — pre-existing seeded data is never touched.
...
Aggregated timing data accumulated across **all** benchmark runs (metrics are never cleared between runs).

**Header:** total entry count + **⬇ CSV** download button

**Table** — one row per `(operation, set_type)` pair:
...
| Avg / P50 / P95 / P99 (µs) | Latency distribution |
| Avg (ms) | Same average in ms for larger ops |

**CSV download** — calls `GET /api/benchmark/export/csv` and triggers a browser file save (`benchmark_metrics.csv`).

**Raw ASCII Table** — expandable section with the backend's own formatted output.

---

## Design System

Dark theme built entirely with CSS custom properties — no external UI library.
...
**Components:** stat cards · filterable tables with pagination · modal dialogs · toast notifications · proportional bar charts · collapsible `<details>` sections · color-coded op breakdown cards.
...
4. Go to Seed Data            → seed 5 000 products
5. Go to Benchmark            → click Run Benchmark
6. Go to Set Inspector        → observe the three different orders live
7. Click View on any product  → see per-set lookup times in nanoseconds
8. Go to Stress Test          → run 20 users × 100 ops
9. Go to Metrics              → download the CSV for offline analysis
```
